...
- 문자열을 초기화 하는 2가지 방법
  - **배열 표기 : `char day1[] = "Monday";`**
    ...
    - 함수 안에서 사용하면 스택 메모리에 저장된다.
      - 함수 안에 있는 `char[]`은 언제나 `스택`에 생긴다. 
      - 이 배열을 초기화할 때, 넣어주는 문자열인 "Monday"는 `const char*`로 `데이터 섹션`에 들어간다.
  ...
  - **포인터 사용 : `char* day2 = "Monday";`**
    - 포인터 변수는 스택에 저장
    - 실제 문자열은 데이터 섹션에 저장
    - **배열 표기와 포인터 사용의 차이점은 실제 문자열이 스택에 안 들어온다는 차이점이다.**
    - 스택에 저장된 문자열은 수정해도 괜찮지만
    - 데이터 섹션에 저장된 문자열은 수정할 경우 크레쉬 날 수 있다.
    - 그래서 **포인터 사용은 문자열 읽기 전용으로 사용한다.**
- 대입
  - 포인터 변수에 값(메모리 주소)을 대입할 수 있으나 배열 변수에는 할 수 없음
  ```c
  int* ptr1;
  int* ptr2;
  int arr1[5];
...
- 포인터와 연산자 우선순위 및 결합법칙 (1)
  - `int num = *p++;` : p++가 우선순위
  - `int num = *(p++);` : **평가 후 실행**
    - `p++`는 p로 **평가** :  아직 p = p + 1 실행 안 함
    - 그래서 `*(p++)`은 *p와 동일 : p의 값(0x104)이 가리키는 주소의 값(134)에 접근
    - `=` 연산자 실행 : num에 값(134)을 대입
    - 평가 후 `p++`을 **실행** : p = p + 1, 0x108이 p에 저장
  - `int num = *(++p);` : **실행 후 평가**
    1. `++p` 는  p = p + 1을 실행 후 평가한다.
    2. *(++p) : 실행 후 평가된 p의 값(0x108)이 가리키는 주소의 값(68)에 접근
    3. `=` 연산자 실행 : num에 값(68)을 대입
- 포인터 연산자 우선순위 및 결합 (2)
  - `int num = ++*p;`
  - `int num = ++(*p);`
